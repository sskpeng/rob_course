...
class MoveArm(object):
    ...
    def is_collision_free(self, q_start, q_goal):
        ...
        while (step_cnt * step_size) < v_length:
            v_step = step_cnt * step_size * v_unit
    ...
    def motion_plan(self, q_start, q_goal, q_min, q_max):
        ...
        #---initialize 
        n_list = []
        n_list.append(Cnode(0,q_start,0))
                
        latest_id = len(n_list)-1
                
        #---loop
        while not self.is_collision_free(n_list[latest_id].position,q_goal):
            print "latest_id=",latest_id        
            #--- sample a random point in C-space
            q_random = numpy.zeros(self.num_joints)
            for j in range(0,self.num_joints):
                q_random[j] = random.random() * (self.q_max[j] - self.q_min[j]) + self.q_min[j]
            # print "q_random=",q_random                        
            
            #--- find a node in the tree that is closed to the random point
            closest_node_id = 0
            closest_distance = numpy.linalg.norm(q_random - q_start)
            for k in range(1,latest_id):
                p_distance = numpy.linalg.norm(q_random - n_list[k].position)
                if p_distance < closest_distance:
                    closest_distance = p_distance
                    closest_node_id = k
                    
            #--- find a point that lies a predefined distance from the existing node to the random point
            step_size = 0.5
            new_v = (q_random - n_list[closest_node_id].position)
            new_point = new_v / numpy.linalg.norm(new_v) + n_list[closest_node_id].position
               
            
            #--- check if the path from the node to the point is collision free
            if self.is_collision_free(new_point,n_list[closest_node_id].position):
                #--- add the new node with the point and with the closest node as its parent
                new_node = Cnode(latest_id+1,new_point,closest_node_id)
                n_list.append(new_node)
                latest_id = len(n_list)-1
                print "new_point=",new_point     
                
            #--- check if the path from the new node to the goal is collision free
            
        #--- once reach the goal, add the goal to the list
        n_list.append(Cnode(latest_id+1,q_goal,latest_id))    
        #--- trace the tree back from the goal to the root and inser each node to the list
        idx = len(n_list)-1
        temp_ID_list = []
        temp_q_list = []
        while n_list[idx].parentID <> 0:
            temp_ID_list.append(n_list[idx].parentID)
            temp_q_list.append(n_list[idx].position)
            idx = n_list[idx].parentID
