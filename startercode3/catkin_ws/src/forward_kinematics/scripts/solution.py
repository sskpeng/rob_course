...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
	    ...
	    #Tw = convert_to_message(T,"word_link","word_link")
        #all_transforms.transforms.append(Tw)
        ...
        for link_name in link_names:
            ...
            jtc = joints[link_idx]
            jtxyz = tf.transformations.translation_matrix(jtc.origin.xyz)
            jtrpy = tf.transformations.translation_matrix(jtc.origin.rpy)
            jtname = jtc.name
            tftemp = numpy.dot(jtxyz,jtrpy)
            ...
            if jtc.type == 'fixed':
                ...
                # revolute joint
                # get vector of the rotational axis
                jt_rotaxis = jtc.axis

                # get the current joint name and joint value
                jt_idx = joint_values.name.index(jtname)
                jt_val = joint_values.position[jt_idx]

                # make the rotation matrix and apply to the full transformation
