...
def cartesian_control(joint_transforms, b_T_ee_current, b_T_ee_desired,
    ...
    T_diff = numpy.dot(ee_T_b_current,b_T_ee_desired)
    b_t_ee = tf.transformations.translation_from_matrix(T_diff)
    b_R_ee = T_diff[:3,:3]
    ee_R_b = ee_T_b_current[:3,:3]
    T_angle, T_axis = rotation_from_matrix(T_diff)
    b_ROT = T_angle * T_axis/numpy.linalg.norm(T_axis)
    
    ### compute (V_ee) = p*x_delta
    x_delta[0] = b_t_ee[0]
    x_delta[1] = b_t_ee[1]
    x_delta[2] = b_t_ee[2]
    x_delta[3] = b_ROT[0]
    ...
    J_temp = numpy.zeros((6,num_joints))
    for i in range(0,num_joints):
        b_T_j = joint_transforms[i]
        ee_T_j = numpy.dot(ee_T_b_current,b_T_j)
        j_T_ee = tf.transformations.inverse_matrix(ee_T_j)
        ee_R_j = ee_T_j[:3,:3]
        j_t_ee = tf.transformations.translation_from_matrix(j_T_ee)
        j_S_ee = S_matrix(j_t_ee)
        rr = tf.transformations.inverse_matrix(ee_R_j)
        j_R_ee = tf.transformations.inverse_matrix(ee_R_j)
        RS = -numpy.dot(ee_R_j,j_S_ee)
        V_j = numpy.zeros((6,6))
        V_j[0,3] = RS[0,0]
        V_j[1,3] = RS[1,0]
        V_j[2,3] = RS[2,0]
    ...
    e = 0.01
    J_pinv = numpy.linalg.pinv(J_temp,e)
    ...
    ### redundant
    if red_control:
        q_dot = numpy.zeros((num_joints,1))
	q_dot[0] = q0_desired - q_current[0]
        dqn = numpy.dot((numpy.identity(num_joints) - numpy.dot(J_pinv,J_temp)),q_dot)
        dq = dq + dqn
    else:
        dq = numpy.dot(J_pinv,V_ee)
